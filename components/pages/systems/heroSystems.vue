...
<!--      <h6  class="hidden md:block w-[90vw] md:w-[500px] text-white text-center pt-6">{{$t(props.heroDescription)}}</h6>-->
      <div v-if="props.buttons" class="flex gap-10 justify-center pt-6">
        <NuxtLink :to="props.buttons.swingDoors.link">
         <primary-button-bigger class="hidden md:block underline"><h2>{{ $t(props.buttons.swingDoors.name)}}</h2></primary-button-bigger>
          <primary-button-bigger class="md:hidden pt-6 md:pt-0 underline"><h3>{{ $t(props.buttons.swingDoors.name)}}</h3></primary-button-bigger>
        </NuxtLink>
        <NuxtLink :to="props.buttons.slidingDoors.link">
          <primary-button-bigger class="hidden md:block underline"><h2>{{ $t(props.buttons.slidingDoors.name) }}</h2></primary-button-bigger>
          <primary-button-bigger class="md:hidden pt-6 md:pt-0 underline"><h3>{{ $t(props.buttons.slidingDoors.name) }}</h3></primary-button-bigger>
        </NuxtLink>
      </div>
    </div>
