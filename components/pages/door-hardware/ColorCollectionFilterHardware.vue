...
function isColorSetAvailable(colorSet) {
  for (const item of filterCount.value.color_collection) {
...
<template class="filter-container ">
  <Disclosure default-open>
    ...
    <DisclosurePanel class="mb-10 mt-6">
      <div
        v-for="collection in colorCollections" :key="collection.id" class="flex gap-x-4 items-center mb-3"
      ...
      >
        ...
        >
          <div :style="{ backgroundColor: collection.hex_code }" class="w-12 h-12 shadow-darkGrey shadow-sm" />
        </div>
        <h4
