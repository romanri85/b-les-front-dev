...
function isMaterialAvailable(material) {
  for (const item of filterCount.value.material) {
...
<template class="filter-container ">
  <Disclosure :key="isNotMobile" :default-open="isNotMobile">
    <DisclosureButton class=" w-full">
      <FilterType filter-name="Выбрать отделку" />
    </DisclosureButton>
    <transition
        ...
        <div class="flex justify-start gap-x-12 w-full pr-4">
          <div v-for="(material) in materialColors" :key="material.material" class="text-primaryDark">
            <h4
              ...
              class="underline-offset-4 font-light" @click="!isMaterialAvailable(material.material) ? null : chooseMaterial(material.material)"
            >
              {{
