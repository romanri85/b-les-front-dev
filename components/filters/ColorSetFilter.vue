...
const { color_sets, activeFilters, materialColors, filterCount } = storeToRefs(filtersStore)
...
function isColorSetAvailable(colorSet) {
  for (const item of filterCount.value.color_set) {
...
<template class="filter-container ">
  <Disclosure :key="isNotMobile" :default-open="isNotMobile">
    <DisclosureButton class=" w-full">
      <FilterType filter-name="Наборы цветов" />
    </DisclosureButton>
    <transition
    ...
    >
      <DisclosurePanel class="mb-10 mt-6">
        <div
          v-for="color_set in filtersStore.color_sets" v-if="filtersStore.color_sets" :key="color_set.id"
          class="flex gap-x-4 items-center mb-3"
          @click="!isColorSetAvailable(color_set.id) ? null : chooseColorSet(color_set.id)"
        >
          <div
            :style="{ borderBottomColor: isColorSetAvailable(color_set.id) && activeFilters.color_set.includes(color_set.id) ? 'black' : 'transparent' }"
            class="pb-1 border-b-4" :class="{
              'cursor-pointer': isColorSetAvailable(color_set.id),
            }"
          >
            <div :style="{ backgroundColor: color_set.hex_code }" class="w-12 h-12 shadow-darkGrey shadow-sm" />
          </div>
          <h4
            :class="{
