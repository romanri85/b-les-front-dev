...
async function updateQueryAndSearch(newQuery: string) {
...
function handleNewMaterialColorProductVariantPage(newPage) {
...
function handleImagePage(newPage) {
            ...
            <div ref="ImageHitsBlock" class=" ">
              <div class="my-image-grid grid grid-cols-3 gap-4">
                <div v-for="(image, index) in ImageSearchResults" :key="index" class="cursor-pointer my-image-cell">
                  <nuxt-img
                    :src="`https://b-les-storage.ams3.digitaloceanspaces.com/media/${image.image}`" class="object-cover cursor-pointer my-image-item"
                    :alt="image.project_name" @click="triggerModal(image)"
                  />
                </div>
              </div>
            </div>
