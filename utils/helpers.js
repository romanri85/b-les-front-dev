...
export function checkIsOpen(workingDaysFrom, workingDaysTo, saturdayFrom, saturdayTo, sundayFrom) {
...
export function generateAddressData(address) {
  return {
    ...
    phone: address.phone,
    email: address.email,
    isEmailShown: address.isEmailShown,
...
export function geoCookieFromStringToObject(escapedJSON) {
  const unescapedString = escapedJSON.replace(/\\"/g, '"').slice(1, -1)
